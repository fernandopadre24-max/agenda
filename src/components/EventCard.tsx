...
import { Briefcase, ArrowUp, ArrowDown, Edit, Trash2, Check, Mic, DollarSign, Loader2, Clock } from 'lucide-react';
  ...
  const renderFinancials = () => {
    if (event.receber) {
      if (event.receber.status === 'pendente') {
        ...
        return (
           <Badge className="bg-green-500 hover:bg-green-500/80 text-white">Recebido</Badge>
        )
      }
    }
    if (event.pagar) {
      if (event.pagar.status === 'pendente') {
        ...
        return (
           <Badge variant="destructive" className="bg-red-600 hover:bg-red-600/80">Pago</Badge>
        )
      }
    }
    return null;
        ...
        <Card className={`hover:border-primary transition-all duration-200 ${event.status === 'realizado' ? 'opacity-60' : ''}`}>
            <div className="flex">
                <Link href={`/events/${event.id}`} className="flex-1">
                    <div className="flex">
                        <div className="flex flex-col items-center justify-center p-2.5 bg-card border-r border-border w-16">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground pt-1 pl-1">
                            ...
                            {time}
                            {event.status === 'realizado' && <Badge variant="outline" className="ml-2">Realizado</Badge>}
                        </div>
                        </div>
                    </div>
                </Link>
