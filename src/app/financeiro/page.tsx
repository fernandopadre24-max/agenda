...
function TransactionList({ title, events, type }: { title: string, events: Event[], type: 'receber' | 'pagar' }) {
    const filteredEvents = events.filter(e => e[type] && e[type]?.status === 'pendente');
        ...
        <div>
            ...
            <div className="space-y-2">
                {filteredEvents.map(event => (
                    <div key={event.id} className="flex justify-between items-center p-2 rounded-lg bg-card">
                        <div>
                            <p className="font-semibold">{event.artista}</p>
                            <p className="text-sm text-muted-foreground">{event.contratante}</p>
                        </div>
                        <Badge variant={type === 'receber' ? 'default' : 'destructive'} className={type === 'receber' ? 'bg-green-600/20 text-green-400 border-green-600/30' : 'bg-red-600/20 text-red-400 border-red-600/30'}>
    ...
    return (
        <div className="flex flex-col min-h-full bg-background">
            ...
            <main className="flex-1 p-4 md:p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <SummaryCard title="A Receber" value={aReceberPendente} icon={Clock} colorClass="text-green-400" />
                   <SummaryCard title="Recebido" value={recebido} icon={CheckCircle} colorClass="text-green-400" />
                   <SummaryCard title="A Pagar" value={aPagarPendente} icon={Clock} colorClass="text-red-400" />
                   <SummaryCard title="Pago" value={pago} icon={CheckCircle} colorClass="text-red-400" />
                </div>
                
                <Separator />

                <div className="space-y-4">
                    <TransactionList title="Próximos Recebimentos" events={events} type="receber" />
                    <TransactionList title="Próximos Pagamentos" events={events} type="pagar" />
                </div>
